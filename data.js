const Storage = {
    ...
    async pushToGithub(jsonData, sha) {
        ...
        if (!res.ok) throw new Error('Gagal push ke GitHub: ' + res.status);
    ...
    async getData() {
        if (this.isGithubEnabled()) {
            try {
                const remote = await this.fetchFromGithub();
                // Simpan ke local juga untuk fallback
                this.saveLocalData(remote.data);
                localStorage.setItem(this.SYNC_KEY, Date.now().toString());
                return remote.data;
            } catch(e) {
    ...
    async addVersion(version) {
        ...
        if (this.isGithubEnabled()) {
            try {
                const remote = await this.fetchFromGithub();
                remote.data.versions.unshift(version);
        ...
        if (this.isGithubEnabled()) {
            try {
                ...
                if (ridx !== -1) {
                    ...
                    remote.data.versions[ridx] = version;
                    await this.pushToGithub(remote.data, remote.sha);
                }
            } catch(e) {
                console.warn('Gagal sync ke GitHub:', e.message);
    ...
    downloadFile(id) {
        ...
        // Increment download (async, fire and forget)
